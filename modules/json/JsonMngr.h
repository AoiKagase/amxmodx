...
class JSONMngr : public IJSONMngr
{
	...
	void _FreeHandle(std::unique_ptr<JSONHandle> &ptr);

	std::vector<std::unique_ptr<JSONHandle>> m_Handles;
	std::deque<JS_Handle> m_OldHandles;
};

extern std::unique_ptr<JSONMngr> JsonMngr;
